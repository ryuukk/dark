...
struct HashMap(K, V, Allocator = Mallocator, bool GCRangesAllowed = true)
{
    ...
    string dump()
    {
        ...
        {
            str ~= "[%5.5d][0x%16.16x][%s][%s]".format(i, _buckets.bs[i].hash, _buckets.bs[i].key,
                    _buckets.bs[i].value);
        }
        return str.join("\n");
    }
    ...
    do
    {
        ...
        do
        {
            ...
            if (h >= ALLOCATED_HASH && (h & HASH_MASK) == hash && keyEquals(_buckets.bs[index].key, key))
            {
                debug (cachetools)
                    safe_tracef("test entry index %d for key %s - success", index, key);
                return index;
            }
            index = (index + 1) & _mask;
        }
    ...
    //
    private long findEmptyIndexExtended(const hash_t start_index, ref BucketStorage buckets, int new_mask) pure const @safe @nogc
    in
    {
    ...
    V get(T)(K k, T defaultValue) const
    {
        ...
        {
            static assert(0,
                    "You must call 'get' with default value of HashMap 'value' type, or with callable, returning HashMap 'value'");
        }
    }
    ...
    /// map[k] = v;
    ///
    void opIndexAssign(K)(V v, K k)do
    {
        put(k, v);
    }
    ///
    ...
    bool remove(K k)
    {
        ...
        assert((_buckets.bs[lookup_index].hash & TYPE_MASK) == ALLOCATED_HASH, "tried to remove non allocated bucket");
        ...
        if (_buckets.bs[next_index].hash == EMPTY_HASH)
        {
            _empty++;
            debug (cachetools)
                safe_tracef("mark index %s empty(%d is empty hash)", lookup_index, next_index);
            ...
            while (free_index != lookup_index)
            {
                ...
                debug (cachetools)
                    safe_tracef("mark index %s empty", free_index);
        ...
        {
            debug (cachetools)
                safe_tracef("mark index %s deleted", lookup_index);
...
version (UNIT_TESTING)
{
    ...
    @safe unittest
    {
        ...
        string[] words = [
            "hello", "this", "simple", "example", "should", "succeed", "or", "it", "should", "fail"
        ];
